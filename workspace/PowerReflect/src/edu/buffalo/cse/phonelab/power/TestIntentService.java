...
//			Log.v(TAG, "The average Data cost is "+averageCostPerByte);
	...
	public void calculateAppPower(int mStatsType, long uSecTime)
	{
		...
		//	Log.v(TAG, "uidStats.size(): " + NU);
			...
			for (int iu = 0; iu < NU; iu++) 
			{
				...
				if (processStats.size() > 0) 
				{
					...
					//	Log.v(TAG, "Process: " + ent.getKey()+ " cputime: "+ cpuTime + " power: " + processPower);
						childformatting(ent.getKey(),processPower);
						
                    
	        		/**/
					}
			//Log.v(TAG, "Max drain of " + highestDrain + " by " + packageWithHighestDrain);
				//	formatting(packageWithHighestDrain,highestDrain);
         ...
         // Calculate Sensor usage
	             ...
	             switch (sensorType) 
	             {
	                ...
	                default:	android.hardware.Sensor sensorData = sensorManager.getDefaultSensor(sensorType);
	                        	if (sensorData != null) 
	                        	{
	                        		...
	                        		parentformatting(sensorData.getName(),multiplier);
          ...
          //  Log.i(TAG,"UID: "+uid+" Power: "+power+" GPStime: "+gpsTime+" WifiTime: "+wifiRunningTimeMs);
            
            parentformatting("Total Power",power);
            parentformatting("ProcessList",childjson);	
            addToLog();
	...
	public void calculateMiscPower(int mStatsType, long uSecTime)
	{
		...
		parentformatting("Uptime since last unplugged",(timeSinceUnplugged / 1000));
        //Add Phone Usage
    	...
    	parentformatting("PhoneUsage",phoneOnPower);
        //Add Screen Usage
        double power = 0;
        ...
        for (int i = 0; i < (Integer)ReflectionUtils.getStaticValue("android.os.BatteryStats","NUM_SCREEN_BRIGHTNESS_BINS"); i++) 
        {
            ...
            {
                Log.i(TAG, "Screen bin power = " + (int) screenBinPower + ", brightnessTime: " + brightnessTime);
            }
        }
        power /= 1000; // To seconds
        //Log.i(TAG,"TotalPowerScreenUsage: "+power);
        parentformatting("ScreenUsage",power);
        ...
        parentformatting("RadioUsage",power);
    	...
    	//formatting("WifiUsage",wifiPower);
       ...
       // Log.i(TAG,"PowerBluetoothUsage"+btPower);
        parentformatting("BluetoothUsage",btPower);
	...
	public void createParentJson()
	{
		parentjson = new JSONObject();
	}

	public void createChildJson()
	{
		childjson = new JSONObject();
	}
	
	public void parentformatting(String tag, Object val)
	{
		try {
			parentjson.put(tag, val);
	...
	public void childformatting(String tag, Object val)
	{
		try {
			childjson.put(tag, val);
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
